...
## 特色功能
1. 自动记忆窗口大小的改变
1. 修改字体（在需要修改的地方点鼠标中键）和大小（在需要修改的地方按住CTRL滚鼠标滚轮）
1. 修改颜色（在需要修改的地方点鼠标右键）
1. 修改背景图片和透明度（在标题栏处点鼠标右键）
1. 修改键位
...
（1.28该行已不使用，但不能删除）文件第二行为主秒表的颜色（1-红 2-橙 3-黄 4-绿 5-青 6-蓝 7-紫 8-白 9-灰），可自行更改；

（1.28该行已不使用，但不能删除）文件第三行为时间表中当前时间小于最佳时间显示的颜色；

（1.28该行已不使用，但不能删除）文件第四行为时间表中当前时间大于等于最佳时间显示的颜色；

文件第五行为自定义的增压神器彩蛋内容；

文件第六行为自定义的求签内容，签与签之间使用“|”分割；
...
98比赛版内核将检测是否开启了《对象浏览器》并显示在主时间左侧；

新增自动计时内核：仙剑98官方Steam

新增自动计时内核：仙剑二官方Steam **【仅支持64位】**（感谢齐小伙和寒泠幽洌的支持，wjjjj12的测试）
