...
重大更新！计时器加入插件功能，某些特殊规则的扩展显示和更多的小功能显示都可以通过插件来进行增量，而不用更新整个计时器了。当前版本插件将可能会使用主界面的标题或者求签或者右下角的区域显示插件的输出，每个位置最多只允许最先加载的插件进行工作和输出。PS：所有插件的源代码也都会进行开源公示；
...
最佳时间线的配置文件的文件名将会带上对应内核的英文名称，比如best.txt变成了bestPAL98.txt；
